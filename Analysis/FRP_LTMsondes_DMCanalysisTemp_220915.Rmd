...
df <- df[which(df$dts >= as.POSIXct(strptime("2021-07-01 00:00", format = "%Y-%m-%d")) & 
                 df$dts <= as.POSIXct(strptime("2021-07-31 23:45", format = "%Y-%m-%d")) &
                 df$Site == "Decker Pool" ),]
...
unique(dfAll$site)
dfAll <- dfAll[which(dfAll$site %in% c("M13_USGS") & # "SacSherman_CDEC", "SDI_USGS","Emmaton_CDEC" ###DMC220713: these stations did not have data
                       dfAll$dts >= as.POSIXct(strptime("2021-07-01 00:00", format = "%Y-%m-%d")) & 
                       dfAll$dts <= as.POSIXct(strptime("2021-07-31 23:45", format = "%Y-%m-%d"))),]
...
dfDeck <- dat3[which(dat3$Station.Code == "Decker Pool" & 
                       dat3$datetimestamp >= as.POSIXct(strptime("2021-07-04 00:00", format = "%Y-%m-%d")) & 
                       dat3$datetimestamp <= as.POSIXct(strptime("2021-07-29 23:45", format = "%Y-%m-%d"))),]
...
dfDeck$frac <- as.integer(rep(c(1:96),25))

dfM13$day <- as.integer(factor(dfM13$Date, levels = unique(dfM13$Date)))
dfM13$frac <- as.integer(c(rep(c(1:96),19),c(1:15,18:96),rep(c(1:96),10)))
