...
rm_Data[nrow(rm_Data)+1,] <- c("Decker Pool","All","2021-11-29 05:52:47",NA)
rm_Data[nrow(rm_Data)+1,] <- c("Decker Pool","All","2021-11-29 06:22:47",NA)
rm_Data[nrow(rm_Data)+1,] <- c("Decker Pool","All","2021-07-03 21:09:18",NA)
rm_Data[nrow(rm_Data)+1,] <- c("Decker Pool","All","2021-07-03 21:24:18",NA)
rm_Data[nrow(rm_Data)+1,] <- c("Decker Pool","All","2021-11-30 12:37:47",NA)
rm_Data[nrow(rm_Data)+1,] <- c("Decker Pool","All","2021-11-30 12:22:47",NA)
rm_Data[nrow(rm_Data)+1,] <- c("Yolo Flyway Farm","All","2021-09-10 08:42:16",NA)
rm_Data[nrow(rm_Data)+1,] <- c("Yolo Flyway Farm","All","2021-10-12 04:27:16","2021-10-12 07:42:16")
rm_Data[nrow(rm_Data)+1,] <- c("Yolo Flyway Farm","All","2022-04-09 19:23:14","2022-04-09 20:23:14")
...
dfcombined.2$dts2 <- lubridate::round_date(dfcombined.2$dts, "15 minutes")
...
dfcombined.2 <- dfcombined.2[-which(dfcombined.2$Depth_m <0),]
...
dfwideDECK.2 <- dfwideDECK[-which(dfwideDECK$Adjacent == 0 | is.na(dfwideDECK$Adjacent)==TRUE),c("dts2","Restoration","Adjacent","Difference")]
dfwideDECK.2$project <- "Decker"
dfwideDECK.3 <- merge(dfwideDECK.2, dfcombined.3[which(dfcombined.3$Site %in% c("Decker Pool","Decker Breach")),c("dts2","Depth_m")], by = "dts2")
...
dfwideFLYW.2 <- dfwideFLYW[-which(dfwideFLYW$Adjacent == 0 | is.na(dfwideFLYW$Adjacent) == TRUE),c("dts2","Restoration","Adjacent","Difference")]
dfwideFLYW.2$project <- "Flyway"
dfwideFLYW.3 <- merge(dfwideFLYW.2, dfcombined.3[which(dfcombined.3$Site == "Yolo Flyway Farm"),c("dts2","Depth_m")], by = "dts2")
...
library(mgcv)
bD.1 <- bam(Difference ~ s(DoY, k =5, bs = "cc"), data = dfwideAll[which(dfwideAll$project == "Decker" & dfwideAll$ToD == "Early"),])
plot(bD.1)
...
plo_deck <- ggplot(dfpred.D, aes(x = DoY, y = hr, z = predvalues, fill = predvalues)) + 
  ...
  geom_text_contour(label.placer = label_placer_flattest(), binwidth = 1) +
...
bF.3 <- bam(Difference ~ te(DoY,hr, resAnom, k = c(14,5,5), bs = c("cc","cc","tp")), data = dfwideFLYW.4)
plot(bF.3)
gam.check(bF.3)
...
plo_flyw <- ggplot(dfpred.F, aes(x = DoY, y = hr, z = predvalues, fill = predvalues)) + 
  geom_tile() + 
  geom_contour(binwidth = 1) + 
  geom_text_contour(label.placer = label_placer_flattest(), binwidth = 0.5) +
  scale_fill_gradientn(colours = c( "blue","white", "red")) +
  ylab("Yolo Flyway Farms\nHour")+
  facet_grid(.~ resAnom) +
  theme_classic2()

grid.arrange(plo_flyw, plo_deck)

ggarrange(plo_flyw + rremove("xlab"), plo_deck, 
          ncol = 1, nrow = 2,align = "h")

dfcombined.4 <- dfcombined.3[-which((dfcombined.3$Site == "Yolo Flyway Farm"
                                     & dfcombined.3$dts2 %ni% dfcombined.3$dts2[which(dfcombined.3$project == "Flyway" &
                                                                                  dfcombined.3$sitetype == "Adj. Waterbody")])
                                    | (dfcombined.3$Site %in% c("Decker Breach","Decker Pool")
                                     & dfcombined.3$dts2 %ni% dfcombined.3$dts2[which(dfcombined.3$project == "Decker" &
                                                                                  dfcombined.3$sitetype == "Adj. Waterbody")])),]
dfcombined.3 <- dfcombined.4

dfordered <- dplyr::arrange(dfcombined.3, project, sitetype, dts2)
ggplot(dfordered, aes(x = date2, y = DO_mgL, color = sitetype)) + geom_point()  + facet_grid(project ~.) #+ ylim(c(0,200))

summThresh <- ddply(dfcombined.3[which(is.na(dfcombined.3$TempC)==FALSE),], c("project","sitetype","Site","season","date2"), summarise,
                    nTemp = length(TempC),
...
ggplot(summThresh[-which(summThresh$project == "Tule"),], aes(x = date2, y = meanTemp, color = sitetype2)) + 
  geom_point() + 
