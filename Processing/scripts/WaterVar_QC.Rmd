...
xSite <- names(list_all_2022)[1:2]
xVar <- c("TempC","DOmgL","SPC","ChlRFU","PCRFU","fDOMRFU","Turb")

list_all_qc <- list()
for(i in 1:length(xSite)){
  df_ts <- do.call(rbind.data.frame, list_all_2022[[xSite[i]]])
...
save(list_all_qc, xSite, xVar, file = here::here("working","VarQC_initial.RData"))
```

## QC1) Flag data outside of reasonable temperature range (1-40C)
```{r 0-40, message = FALSE}
rm(list = ls())
load(here::here("working","VarQC_initial.RData"))

df_ranges <- data.frame(xVar = xVar,
                        range_low = c(1,1,100,0.01,0.01,1,1),
                        range_high = c(40,25,55000, 100,100,100,1000))
...
save(list_all_QC1, xSite, xVar, file = here::here("working","VarQC_step1.RData"))
```
...
save(list_all_QC2, xSite, xVar, file = here::here("working","VarQC_step2.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_step2.RData"))
#########################################################
...
save(list_all_QC3, xSite, xVar, file = here::here("working","VarQC_step3.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_step3.RData"))

df_Trend <- data.frame(xVar = xVar,
                        xTrend = c("6 months","6 months","6 month","7 days","7 days","15 days","6 months"))
...
save(list_all_QC4, xSite, xVar, file = here::here("working","VarQC_step4.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_step4.RData"))

df_spike <-  data.frame(xVar = xVar,
                        xSpike = c(5,5,250,5,5,25,25))
...
save(list_all_QC5, xSite, xVar, file = here::here("working","VarQC_step5.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_step5.RData"))

df_Rate <-  data.frame(xVar = xVar,
                        xSDth = c(5,5,5,5,5,5,5))
...
save(list_all_QC6, xSite, xVar, file = here::here("working","VarQC_step6.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_step1.RData"))
load(here::here("working","VarQC_step6.RData"))
...
save(list_all_QC7, xSite, xVar, file = here::here("working","VarQC_step7.RData"))
...
for(i in 1:length(xSite)){
  ...
  ggsave(paste("ContinuousWQ_",xSite[i],".png",sep = ""), multi_plo, width = 625, height = 800, dpi = 96, units = "px")
...
save(list_all_QC7, xSite, xVar, file = here::here("final","QCd2_Data_FRP.RData"))
```
