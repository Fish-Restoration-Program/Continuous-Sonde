...
dfUserPaths <- data.frame(userX = c("Danny Cox", "Emma Mendonsa", "Brian Armstrong"),
                          pathX = c("C:/Users/DCox/California Department of Water Resources/DWR - Fish Restoration Program - Data Downloads", 
...
pull2Git <- function(user){
  ...
  pathX = dfUserPaths$pathX[which(dfUserPaths$userX == user)] # Chooses path based on user input
  dirX <- data.frame(shared = list.files(path = pathX), # makes a data frame of paths for source and GitHub directory for each station
                     gitX = NA)
  # The next line adds the GitHub paths. Note that this may change when Stations get added
  dirX$gitX <- c(NA, path2_ARNO_Int, path2_ARNO_Ext, path2_BRAD_Int, path2_BRAD_Ext, path2_DECK_Breach, path2_DECK_Pool, path2_LOWE_EastInt, path2_LOWE_Stairsteps, NA, NA, path2_TULE_Breach, path2_TULE_Main, path2_TULE_PondC, path2_WING, path2_WINT, path2_FLYW)
  # This for loop adds each file that is not already present in the GitHub repository
  for(i in 1:nrow(dirX)){
    if(is.na(dirX$gitX[i])==F){ # Proceed if and only if there is a GitHub path
      filesShare <- list.files(path = paste(pathX,dirX$shared[i],"CSV",sep = "/")) # make a list of files in SharePoint folder
      filesGit <- list.files(dirX$gitX[i]) # make a list of files in GitHub folder
      ...
      if(length(notGit) > 0){ # Proceed if there are files that are missing
        files2get <- paste(pathX,dirX$shared[i],"CSV",notGit,sep = "/") # make path for file in SharePoint
        file.copy(file.path(files2get), dirX$gitX[i]) # copy the file over to GitHub
        print(paste("Copied over",notGit)) # Confirmation message that the file was copied over
      } else (print(paste("No new files found in", dirX$shared[i]))) # This message appears if the GitHub repository is up-to-date with SharePoint
    }
  }
}
...
#   print("Hello Danny")
#   pathX <- c("C:/Users/DCox/California Department of Water Resources/DWR - Fish Restoration Program - Data Downloads")
#   dirX <- data.frame(shared = list.files(path = pathX),
#                      gitX = NA)
#   dirX$gitX <- c(NA, path2_ARNO_Int, path2_ARNO_Ext, path2_BRAD_Int, path2_BRAD_Ext, path2_DECK_Breach, path2_DECK_Pool, path2_LOWE_EastInt, path2_LOWE_Stairsteps, NA, NA, path2_TULE_Breach, path2_TULE_Main, path2_TULE_PondC, path2_WING, path2_WINT, path2_FLYW)
#   for(i in 1:nrow(dirX)){
#     if(is.na(dirX$gitX[i])==F){
#       filesShare <- list.files(path = paste(pathX,dirX$shared[i],"CSV",sep = "/"))
#       filesGit <- list.files(dirX$gitX[i])
#       notGit <- filesShare[which(filesShare %ni% filesGit)]
#       if(length(notGit) > 0){
#         files2get <- paste(pathX,dirX$shared[i],"CSV",notGit,sep = "/")
#         file.copy(file.path(files2get), dirX$gitX[i])
#         print(paste("Copied over",notGit))
#       } else (print(paste("No new files found in", dirX$shared[i])))
#     }
#   }
