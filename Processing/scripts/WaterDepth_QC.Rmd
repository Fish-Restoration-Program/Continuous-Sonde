...
dfcombined.2$Depth_m.2 <- ifelse(is.na(dfcombined.2$Depth_m)==TRUE,
                               dfcombined.2$Depth_ft*0.3048,
                               dfcombined.2$Depth_m)
...
temp_H_0 <- temp_H[,c("Site","dts","Depth_m.2","date","hour","minute")]
...
temp_q1 <- temp_H %>% mutate(Flag_QC1 = ifelse(Temp<0 | Temp>10, "Y", "N")) 
...
temp_q3 <- repeating_vals(df = temp_q2, x = 72) %>% # 18 for hourly, 72 for quarter-hourly
...
temp_q5 <- temp_q4 %>%
  ...
  mutate(Flag_QC5 = ifelse((QC5 > 1), "Y", "N"))  %>% # changed to 1m
...
depth_final <- temp_final
save(depth_final, file = "AllLTM4AnalysisQC_DepthQH_221230.RData")
```
