---
title: "FRP_ContinuousWQ_SondeWorksheets for 2024 rating data"
author: "DMC"
date: "6/23/2023"
...
cw_Exch <- data.frame(read.csv(here::here("raw","Survey123", "Exchanges_v240206", "Exchanges240206_ColumnCrosswalk.csv")))
...
df_Exch <- data.frame(read.csv(here::here("raw","Survey123", "Exchanges_v240206", "Exchanges240206_Export250627.csv")))
...
## Did not use the two forced date change tasks for 2024 data.  - ALM
...
save(df_DeployTimes, df_RecoveryTimes, file = here::here("working","Processing_2024","CDFW_ExchangeTimes.RData")) #  probably won't need this for 2024 prcessing but saved it anyway.   - ALM
# save the exchange worksheet for use in 'ErrorCalculations.Rmd'
save(df_Exch, file = here::here("working","Processing_2024","CDFW_ExchangeWorksheet.RData"))
```
...
rm(list = ls())
'%ni%' = Negate('%in%')
cw_Calib <- data.frame(read.csv(here::here("raw","Survey123","Calibrations_v240206", "Calibration240602_ColumnCrosswalk.csv")))
...
df_Calib <- data.frame(read.csv(here::here("raw","Survey123", "Calibrations_v240206", "Calibration240206_Export250627.csv")))

colnames(df_Calib) <- cw_Calib$Var_Name

# fill missing deployment times by using values from the calibration time  - ALM
df_Calib <- df_Calib %>% mutate(dts_startstop = ifelse(deployment_startstop_time %in% "", calibration_date_and_time, deployment_startstop_time))

# make a datetime object for the deployment start/stop time and the calibration time   
df_Calib$dts_startstop <- format(lubridate::mdy_hms(df_Calib$dts_startstop), tz = "America/Los_Angeles")

# make 2 separate dataframes (1 with just the data we are interested and 1 with the metadata)
df_CalibData <- df_Calib[,c("GlobalID","sonde_id","sonde_other","discrete_or_continuous","pre_post","dts_startstop",colnames(df_Calib)[53:107])]
df_CalibMeta <- df_Calib[,c("GlobalID","sonde_id","sonde_other","discrete_or_continuous","pre_post","dts_startstop",colnames(df_Calib)[c(7:52,109:114)])]

save(df_Calib, df_CalibData, df_CalibMeta, file = here::here("working","Processing_2024","CDFW_CalibrationWorksheet.RData"))
```
#Did not run any code from here down for 2024 data. - ALM
