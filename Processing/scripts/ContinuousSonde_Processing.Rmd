...
pathnames <- data.frame(SiteName = c("FLYW","DECK_Pool","DECK_Breach","TULE_Breach","TULE_PondC","ARNO_Exterior","ARNO_Interior","BRAD_Exterior"), 
                        pathname = c(path2_FLYW, path2_DECK_Pool, path2_DECK_Breach, path2_TULE_Breach, path2_TULE_PondC, path2_ARNO_Ext, path2_ARNO_Int, path2_BRAD_Ext))
...
for(i in 1:nrow(fname)){
  ...
  firstObs <- as.POSIXct(strptime(paste(df_temp$Date[1],df_temp$Time[1],sep = " "), format = "%m/%d/%Y %H:%M:%S"), tz = "America/Los_Angeles")
  lastObs <- as.POSIXct(strptime(paste(df_temp$Date[nrow(df_temp)],df_temp$Time[nrow(df_temp)],sep = " "), format = "%m/%d/%Y %H:%M:%S"), tz = "America/Los_Angeles")
...
for(i in 1:length(list_all_raw)){
  for(j in 1:length(list_all_raw[[i]])){
    names(list_all_raw[[i]])[j] <- paste(list_all_meta[[i]][[j]]$ExoValue[which(list_all_meta[[i]][[j]]$ExoVariable == "Sonde")],
                                         ...
                                         sep = " ")
      list_all_raw[[i]][[j]]$dts <- as.POSIXct(strptime(paste(list_all_raw[[i]][[j]]$Date,list_all_raw[[i]][[j]]$Time,sep = " "), format = "%m/%d/%Y %H:%M:%S"), tz = "America/Los_Angeles")
  }
}
