...
for(i in 1:length(xSite)){
  ...
  for(j in 1:length(xVar)){
    ...
    df_q1$Flag_QC1 <- ifelse(as.numeric(df_q1$Var) < df_ranges$range_low[j] |
                               as.numeric(df_q1$Var) > df_ranges$range_high[j],
                             "Y","N")
...
for(i in 1:length(xSite)){
  ...
  ggsave(paste("ContinuousWQ_",xSite[i],".png",sep = ""), path = here::here("figures","diagnostics"), multi_plo, width = 625, height = 800, dpi = 96, units = "px")
...
save(list_all_QC7, xSite, xVar, file = here::here("working","QCd2_Data.RData"))
```
