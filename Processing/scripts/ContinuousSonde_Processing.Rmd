...
for(i in 1:length(list_all_raw)){
  for(j in 1:length(list_all_raw[[i]])){
    names(list_all_raw[[i]])[j] <- paste(list_all_meta[[i]][[j]]$ExoValue[which(list_all_meta[[i]][[j]]$ExoVariable == "Sonde")],
                                         ...
                                         sep = " ")
    list_all_raw[[i]][[j]]$dts <- as.POSIXct(strptime(paste(list_all_raw[[i]][[j]]$Date,list_all_raw[[i]][[j]]$Time,sep = " "), format = "%m/%d/%Y %H:%M:%S"))
  }
}
...
Now, we will bring in the sensor ratings and associate them with each dataset. Ratings were calculated in the 'ErrorCalculations.Rmd' script
...
df_VarNames$VarName
list_ratings[[1]]$varX
df_xNames <- data.frame(VarName = c("TempC","DOmgL","SPC","ChlRFU","PCRFU","fDOMRFU","Turb"),
                        ErrorName = c("temp","DO", "sc","chl","pc","fdom","turb"),
                        qcName = c("TempC_qcE","DOmgL_qcE","SPC_qcE","ChlRFU_qcE","PCRFU_qcE","fDOMRFU_qcE","Turb_qcE"))
...
library(plyr)
summ_2022 <- ddply(allRatings[-which(allRatings$varX %in% c("pH","DO_sat","sc_InAir")),], c("varX","Rating_final"), summarise,
                   v = sum(nObs, na.rm = TRUE))
tot_2022 <- ddply(allRatings, c("sondeID","dateRecov"), summarise,
                   v = sum(nObs, na.rm = TRUE),
...
summ_2022$varX_F <- factor(summ_2022$varX, levels = c("temp","sc","DO","turb","chl","pc","fdom"))

library(ggplot2)
plo_summ <- ggplot(summ_2022, aes(varX_F, propX)) + geom_bar(aes(fill = Rating_finalF), stat = "identity")
plo_summ

save(list_all_qc, file = here::here("final","QCd_Data_FRP.RData"))
```

```{r}
rm(list = ls())
load(here::here("final","QCd_Data_FRP.RData"))
...
for(i in 1:length(list_all_qc)){
  ...
  for(j in 1:length(list_all_2022[[i]])){
    rtd_sondeID <- strsplit(names(list_all_2022[[1]])[1], split = " ", fixed = TRUE)[[1]][1] # variable for sondeID
    rtd_dateDeploy <- strsplit(names(list_all_2022[[1]])[1], split = " ", fixed = TRUE)[[1]][2] # variable for Deployment date
    rtd_dateRecov <- strsplit(names(list_all_2022[[1]])[1], split = " ", fixed = TRUE)[[1]][3] # variable for Recovery date
    ###DMC230721: pick up here and get 2022 based on dates in preceding two lines
    # fill in rating columns of deployment dataframe with the Final Rating
    for(a in 1:ncol(df_temp)){
      for(b in 1:nrow(df_xNames)){
        if(colnames(df_temp)[a] == df_xNames$VarName[b]){
  ...
  names(list_qc) <- names(list_all_trim2[[i]])
  list_all_2022[[i]] <- list_qc
  rm(list_qc, rtd_sondeID, rtd_dateDeploy, rtd_dateRecov)
}
names(list_all_2022) <- names(list_all_trim2)
```
