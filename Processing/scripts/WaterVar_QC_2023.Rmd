...
---
Modified by Daniel Cox, CDFW in July 2024 from original script written by Catarina Pien May 2020.
...
load(here::here("working","QCd_2023.RData"))

# Check which datasets have data
names(list_all_2023)
xSite <- names(list_all_2023)[-c(3,4,12)] # Do not have data so will not include in the downstream processes
...
for(i in 1:length(xSite)){
  ...
  # }
  #df_ts <- do.call(rbind.data.frame, list_site)
  list_site <- list_all_2023[[xSite[i]]]
  df_ts <- do.call(rbind.data.frame, list_site)
  ...
  df_ts <- df_ts[-which(df_ts$Depthm <0 | 
                          df_ts$SPC < 100 | 
                          (df_ts$dts < as.POSIXct(strptime("2023-01-01 00:00:00", format = "%Y-%m-%d %H:%M:%S"))|
                             df_ts$dts > as.POSIXct(strptime("2023-12-31 23:59:59", format = "%Y-%m-%d %H:%M:%S")))),]
  list_qc <- list()
...
save(list_all_qc, xSite, xVar, file = here::here("working","VarQC_2023","VarQC_initial.RData"))
```

## QC1) Flag data outside of reasonable temperature range (1-40C)
```{r 0-40, message = FALSE}
rm(list = ls())
load(here::here("working","VarQC_2023","VarQC_initial.RData"))
...
save(list_all_QC1, xSite, xVar, file = here::here("working","VarQC_2023","VarQC_step1.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_2023","VarQC_step1.RData"))
...
save(list_all_QC2, xSite, xVar, file = here::here("working","VarQC_2023","VarQC_step2.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_2023","VarQC_step2.RData"))
#########################################################
...
save(list_all_QC3, xSite, xVar, file = here::here("working","VarQC_2023","VarQC_step3.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_2023","VarQC_step3.RData"))
...
save(list_all_QC4, xSite, xVar, file = here::here("working","VarQC_2023","VarQC_step4.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_2023","VarQC_step4.RData"))
...
save(list_all_QC5, xSite, xVar, file = here::here("working","VarQC_2023","VarQC_step5.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_2023","VarQC_step5.RData"))
...
save(list_all_QC6, xSite, xVar, file = here::here("working","VarQC_2023","VarQC_step6.RData"))
```
...
rm(list = ls())
load(here::here("working","VarQC_2023","VarQC_step1.RData"))
load(here::here("working","VarQC_2023","VarQC_step6.RData"))
...
save(list_all_QC7, xSite, xVar, file = here::here("working","VarQC_2023","VarQC_step7.RData"))
...
rm(list = ls())
load(here::here("working","VarQC_2023","VarQC_step1.RData"))
load(here::here("working","VarQC_2023","VarQC_step7.RData"))
...
for(i in 1:length(xSite)){
  ...
  names(list_plo) <- xVar
  # This ifelse statement allows CDFW sites to plot all variables except pH while DWR sites plot all variables except fDOM. Starting in 2023, DWR sites will have Chl and PC (except exterior station for Arnold)
  if(xSite[i] %in% c("FLYW", "DECK_Pool")){
    multi_plo <- ggarrange(list_plo[[1]], list_plo[[2]], list_plo[[3]], list_plo[[4]], list_plo[[5]], list_plo[[6]], list_plo[[7]],
                         ncol = 1, nrow = 7)
                         #common.legend = TRUE, legend = "bottom")
  ggsave(paste("ContinuousWQ_2023_",xSite[i],".png",sep = ""), multi_plo, width = 625, height = 800, dpi = 96, units = "px", path = here::here("figures","VarQC_2023"))
  } 
  if(xSite[i] %in% c("TULE_PondC","TULE_Main","ARNO_Interior","BRAD_Exterior","BRAD_Interior","LOWE_EastInt","WING","WINT")){
  multi_plo <- ggarrange(list_plo[[1]], list_plo[[2]], list_plo[[3]], list_plo[[4]], list_plo[[5]], list_plo[[7]], list_plo[[8]],
                         ncol = 1, nrow = 7)
                         #common.legend = TRUE, legend = "bottom")
  ggsave(paste("ContinuousWQ_2023_",xSite[i],".png",sep = ""), multi_plo, width = 625, height = 800, dpi = 96, units = "px", path = here::here("figures","VarQC_2023"))
  }
  if(xSite[i] %in% c("ARNO_Exterior")){
...
save(list_all_QC7, xSite, xVar, file = here::here("final","QCd2_Data_2023.RData"))
```
