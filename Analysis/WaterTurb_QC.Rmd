...
dfcombined.2$hour <- lubridate::hour(dfcombined.2$dts)
dfcombined.2$minute <- lubridate::minute(dfcombined.2$dts)
dfcombined.2$year <- lubridate::year(dfcombined.2$dts)
dfcombined.2 <- dfcombined.2[-which(dfcombined.2$Depth_m <0 | dfcombined.2$year %in% c(2019,2020)),]
...
rm_Data[nrow(rm_Data)+1,] <- c("Decker Pool","All","2021-06-16 08:09:18",NA) # overlap with subsequent deployment (likely in bucket)
rm_Data[nrow(rm_Data)+1,] <- c("Decker Pool","All","2021-09-10 07:23:16",NA) # overlap with subsequent deployment (likely in bucket)
...
for(i in 1:nrow(rm_Data)){
  if(is.na(rm_Data$EndDate[i])==TRUE){
    dfcombined.2[which(dfcombined.2$Site == rm_Data$Site[i]
                         & dfcombined.2$dts == rm_Data$StartDate[i]),] <- NA
  } else {
    dfcombined.2[which(dfcombined.2$Site == rm_Data$Site[i] 
                         & (dfcombined.2$dts >= rm_Data$StartDate[i]
                            & dfcombined.2$dts <= rm_Data$EndDate[i])),] <- NA
  }
...
temp_H <- dfcombined.2 %>%
  ...
  #slice(1) %>% #keep only the first value for each station, date, hour group so 1 value/hour # turn off for quarter-hourly
  ungroup()

temp_H_0 <- temp_H[,c("Site","dts","Turb_FNU","date","hour", "minute")]
colnames(temp_H_0) <- c("Station","Datetime","Temp","Date","Hour","Minute")
...
rm(temp_H_0, dfcombined.2)

```

## QC1) Flag data outside of reasonable turbidity range (0.1-200C)
```{r 0-40, message = FALSE}

# Data including flags
temp_q1 <- temp_H %>% mutate(Flag_QC1 = ifelse(Temp<0.1 | Temp>200, "Y", "N")) 
...
temp_q3 <- repeating_vals(df = temp_q2, x = 92) %>% # 23 for hourly, 92 for quarter-hourly
...
temp_q5 <- temp_q4 %>%
  ...
  mutate(QC5 = abs(Temp- 0.5 * (lag(Temp, n = 4, default = 0) + lead(Temp, n=4, default = 0))))%>% # n = 1 for hourly, n = 4 for quarter-hourly
  mutate(Flag_QC5 = ifelse((QC5 > 25), "Y", "N"))  %>%
...
temp_q6 <- temp_q5 %>%
  group_by(Station) %>%
  arrange(Station, Datetime) %>%
  mutate(QC6 = abs(Temp- lag(Temp, n = 4, default = 0)))%>% # n = 1 for hourly, n = 4 for quarter-hourly
  mutate(sdev_th = 5 * runSD(Temp, 200))%>% # n = 50 for hourly, n = 200 for quarter-hourly
...
ggplot(temp_final, aes(x = Datetime, y = Temp)) + geom_point() + facet_grid(Station ~., scales = "free_y")

turb_final <- temp_final
save(turb_final, file = "AllLTM4AnalysisQC_TurbQH_230102.RData")
```
