...
plo_turb <- ggarrange(heatFLYW, heatDECK, legendT,
                      heatFLYW.diff, heatDECK.diff +rremove("legend"), legendT.Diff,
                      ncol = 3, nrow = 2,
                      align = "h",
                      widths = c(2,2,0.8))

ggsave(paste("Heatmap_Turb",".png",sep = ""), path = here::here("figures","publication"), plo_turb, width = 1600, height = 1080, dpi = 192, units = "px", bg="white")
```
...
dfwideFLYW <- dfwideAll[which(dfwideAll$project == "FLYW"),]
bF.2 <- bam(Restoration ~ te(DoY, hr, bs = c("cc","cc")), data = dfwideFLYW)
plot(bF.2)
gam.check(bF.2)
dfwideFLYW$resAnom <- residuals(bF.2, type = "response")

bF.3 <- bam(Difference ~ te(DoY,hr, resAnom, bs = c("cc","cc","tp")), data = dfwideFLYW)
plot(bF.3)
gam.check(bF.3)
...
plo_DO <- ggarrange(heatFLYW, heatDECK, legendT,
                    heatFLYW.diff, heatDECK.diff +rremove("legend"), legendT.Diff,
                    ncol = 3, nrow = 2,
                    align = "h",
                    widths = c(2,2,0.8))

ggsave(paste("Heatmap_DO",".png",sep = ""), path = here::here("figures","publication"), plo_DO, width = 1200, height = 900, dpi = 168, units = "px", bg="white")
```
