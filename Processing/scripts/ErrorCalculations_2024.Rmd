...
allRatings.2c <- allRatings.2[which(is.na(allRatings.2$Rating_final)==TRUE),] # double check NAs
library(tidyverse)
check_na <- allRatings.2c %>% count(dateRecov,sondeID)   # includes 2025 discrete deployments (22)
tst <- allRatings.2c %>% filter(year(dateRecov)%in%"2024",varX%in%c("pH","temp","DO","chl","pc","fdom","turb"))  # keep only 2024 records and remove sc_InAir & DO_sat variables
check_na_2024 <- tst %>% count(dateRecov,sondeID)  
check_na_2024$notes <- c("pH: no calibration","temp: not recorded","temp: not recorded","temp: not recorded, pH: no calibration","do: pre-cal do_mgL not recorded","pH: no calibration","calibration for sc only - no other parameters calibrated","pH & turb: two records exist","pH: no calibration","do: pre-cal do_mgL not recorded")
